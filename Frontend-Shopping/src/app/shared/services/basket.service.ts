...
export class BasketService {
  ...
  public addToBasket(product: IProduct, size: string, color: string): void {
    let productVar: IProduct[] = this.basket.map(a => a.product)
    let prdouctIsExsist = productVar.find(({ id }) => id == product.id)

    if (!prdouctIsExsist) {
      this.basket.push({ count: 1, product: product, size, color });
      localStorage.setItem('basket', JSON.stringify(this.basket));
      this.isChanged.next(true);
      this.notifier.notify("success", "Added to basket")
    }
